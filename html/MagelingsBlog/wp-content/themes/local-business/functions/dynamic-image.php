 ...
 * @return mixed|string
 */
function localbusiness_document_root($path) {
 ...
 */
function localbusiness_image_resize($img_url, $width, $height, $crop = true, $quality = 100) {
    ...
    if ( !empty( $file_path['host'] ) && $_SERVER['HTTP_HOST'] != $file_path['host'] ) {  // The image is not locally hosted
        ...
        $file_path = localbusiness_document_root($file_path['path']) . $file_path['path'];
    }
 ...
 * @return type 
 */
function localbusiness_thumbnail_resize($attach_id = null, $img_url = null, $width, $height, $crop = false, $jpeg_quality = 90) {
