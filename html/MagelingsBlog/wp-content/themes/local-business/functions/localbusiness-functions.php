...
add_action('init', 'localbusiness_register_custom_menu');

function localbusiness_register_custom_menu() {
...
function localbusiness_breadcrumbs() {
    $delimiter = '&raquo;';
    ...
    if (get_query_var('paged')) {
        if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
            echo ' (';
        echo __('Page', 'local-business') . ' ' . get_query_var('paged');
        if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
            echo ')';
    }
    echo '</div>';
 ...
 * @param type $ih 
 */
function localbusiness_get_thumbnail($iw, $ih) {
    $permalink = get_permalink();
    $thumb = get_post_thumbnail_id();
    $image = localbusiness_thumbnail_resize($thumb, '', $iw, $ih, true, 90);
 ...
 */
function localbusiness_get_image($width, $height) {
    $w = $width;
    $h = $height;
    global $post, $posts;
//This is required to set to Null
    ...
    $img_path = localbusiness_image_resize($img_source, $w, $h);
...
/** Register sidebars by running localbusiness_widgets_init() on the widgets_init hook. */
...
function localbusiness_childtheme_analytics() {
    $output = localbusiness_get_option('localbusiness_analytics');
    if ($output <> "")
        echo stripslashes($output) ;
}

add_action('wp_head', 'localbusiness_childtheme_analytics');
/* ----------------------------------------------------------------------------------- */
/* Custom CSS Styles */
/* ----------------------------------------------------------------------------------- */

function localbusiness_of_head_css() {
    $output = '';
    $custom_css = localbusiness_get_option('localbusiness_customcss');
    if ($custom_css <> '') {
        $output .= $custom_css . "\n";
    }
// Output styles
...
add_action('wp_head', 'localbusiness_of_head_css');
